...
def test_lammpsstate_varntypat():
    """
    Test that LammpsState can do a MD on H2O, H2O2 and CH4
    Try with MLIAP/SNAP/MTP/ACE
    """
def generate_atoms():
    # Create_H2O
...
def test_predict_varntypat(root, mlip_fn):
    ...
    if fn.suffix == ".model":  # SNAP or MLIAP
        fn = mlip.descriptor.get_coef(fn)
    elif fn.suffix == ".mtp":  # MTP
        Path("MTP").mkdir(exist_ok=True)
        mtp_fn = Path("MTP") / fn.name
        shutil.copy(fn, mtp_fn)
        with open(Path("MTP") / "mlip.ini", "w") as f:
            f.write(f"mtp-filename    {mtp_fn.absolute()}\n")
            f.write("select          FALSE")
    elif fn.suffix == ".yace":  # ACE
        Path("ACE").mkdir(exist_ok=True)
        shutil.copy(fn, Path("ACE") / "ACE.yace")
    mlip.next_coefs(fn)
    for molecule in atoms:
        res = state.run_dynamics(supercell=molecule,
...
#def test_lammpsstate_varntypat():
#    """
#    Test that a MLIAP/SNAP/MTP/ACE can execute lammps on H2O and CH4
#    """
#    pass
#    atoms = create_h2()
#    
