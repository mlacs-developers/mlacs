...
def PathIntegralTrajectory(prefix, mode="r", qpolymer=None, properties=None, 
                           master=None, nbeads=None):
    """
    """
    if mode == "r":
        return PathIntegralTrajectoryReader(prefix, nbeads)
    return PathIntegralTrajectoryWriter(prefix, mode, qpolymer, properties, master=master, nbeads=nbeads)
...
class PathIntegralTrajectoryWriter:
    ...
    def __init__(self, prefix, mode="w", qpolymer=None, properties=None, 
        ...
        for ibead in range(self.nbeads):
            if qpolymer is None:
                self._traj.append(Trajectory(prefix + "_{}.traj".format(ibead), "w", None,
                                       properties, master=master))
            else:
                self._traj.append(Trajectory(prefix + "_{}.traj".format(ibead), "w", qpolymer[ibead],
                                       properties, master=master))
...
class PathIntegralTrajectoryReader:
    ...
    def __init__(self, prefix, nbeads):
        ...
        self.nbeads = nbeads
        for ibead in range(self.nbeads):
            self._traj.append(Trajectory(prefix+"_{0}.traj".format(ibead), "r"))
