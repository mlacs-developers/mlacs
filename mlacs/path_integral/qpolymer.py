...
class qPolymer:
    ...
    def get_potential_energy(self, force_consistent=False, apply_constraint=True):
        ...
        for ibead in range(self.nbeads):
            energy[ibead] = self[ibead].get_potential_energy(force_consistent, apply_constraint)
        return energy
    ...
    def get_stress(self, voigt=True, apply_constraint=True, include_ideal_gas=False):
        ...
        if voigt is False:
            ...
            for ibead in range(self.nbeads):
                stress[ibead] = self[ibead].get_stress(voigt, apply_constraint, include_ideal_gas)
        else:
    ...
    def set_center_of_mass(self, com, scaled=False):
        ...
        diff    = old_com - com
        if scaled:
            self.set_scaled_positions(self.get_scaled_positions() + diff[:,None,:])
        else:
    ...
    def set_positions(self, newpositions, apply_constraint=True):
        ...
        if len(np.shape(newpositions)) == 2:
            ...
            for ibead in range(self.nbeads):
                self[ibead].set_positions(newpositions[ibead], apply_constraint)
        else:
            raise ValueError("Positions can only be a natoms X 3 or a nbeads X natoms X 3 array")
    ...
    def get_staging_positions(self):
        ...
        tpos[self.nbeads-1] = pos[self.nbeads-1] - ((self.nbeads - 1) * pos[0] + pos[0]) / (self.nbeads)
        return tpos
