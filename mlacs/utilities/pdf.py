...
#========================================================================================================================#
'''
Function to compute the pair distribution function given a list of atoms files
can handle weights.
Can use a counting method or force-sampling method (default)
'''
#========================================================================================================================#
def compute_pdf(configurations, temperature=None, weights=None, fname=None, **kwargs):
    '''
    ...
    method: str
       Method to compute the pdf, available options are 'count' and 'force_sampling'
    nbins: int
    ...
    if method == 'force_sampling':
        if temperature is None:
            msg = 'The temperature is needed to compute the pdf with the force sampling method'
    ...
    nl     = NeighborList([rcut / 2.] * natoms, self_interaction=False)#, bothways=True)
    ...
    for iconf in range(nconfs):
        if np.abs(weights[iconf]) >= 1e-8:
            ...
            # We need the chemical symbols to be able to compute a per-interaction g(R)
            ...
            for iat in range(natoms):
                ...
                rij = atoms.positions[jat_idx] + np.dot(offsets, atoms.cell) - atoms.positions[iat]
                dij = np.sqrt(np.sum(rij**2, axis=1))
         
                iat_el = np.where(elements==symbols[iat])[0][0]
         
                count_iat = np.zeros_like(count)
                # If counting method, we use the histogram function of numpy #TODO add the multi-element case
                if method == 'count':
                    for el in range(nelements):
                        el_idx = np.where(symbols[jat_idx]==elements[el])[0]
                        count_iat_tmp, edges_iat = np.histogram(dij[el_idx], bins=nbins, range=(0.0, rcut))
                        inte            = idx_int[iat_el, el]
                        count_iat[inte] = count_iat_tmp
         
                if method == 'force_sampling':
                    f_jat = forces[jat_idx]
                    tmp   = -0.5 * (forces[iat] - f_jat) * rij / (dij**3).repeat(3).reshape(-1,3)
                    #count_iat = np.zeros_like(count)
                    # Loop on the pdf distances
                    for n in range(len(points)):
                        for el in range(nelements):
                            # We need the interactions between the elements of iat and element el
                            tmp_el_idx = np.where(symbols[jat_idx]==elements[el])[0]
                            tmp_el     = tmp[tmp_el_idx]
                            # inte is the index allowing to place the interaction the right place
                            inte       = idx_int[iat_el, el]
                            # We extract only the distances for this type of interaction
                            dij_el     = dij[tmp_el_idx]
                            # In the force-sampling formulation, all atoms within a distance inferior to R contributes
                            count_iat[inte,n] = np.sum(tmp_el[dij_el < points[n]])
         
                # Finally we weight the contribution
                # The 2 is to account for the bothways=False in the neighborlist
    ...
    # Normalize the pdf
    if method == 'count':
        count  *= norm / (points**2) * natoms #TODO make the normalization right
    ...
    # If asked, we write the pdf, with an indication of what each column represent
    if fname is not None:
        header = "Distances            "
        for i in range(nint):
            idx= np.where(i==idx_int)[0]
            if len(idx) > 1:
                header += "{:}-{:}                  ".format(elements[idx[0]], elements[idx[1]])
            else:                              
                header += "{:}-{:}                  ".format(elements[idx[0]], elements[idx[0]])
        if nelements > 1:
            header += "tot"
